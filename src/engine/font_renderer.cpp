...
FontRenderer::FontRenderer(GfxState *gfx) 
{
    this->freetype_init();
    this->init_render_resources(gfx);
    this->alloc_font_memory();
...
bool FontRenderer::init_atlas_texture(GfxState* gfx) 
{
...
void FontRenderer::init_render_resources(GfxState* gfx) {
    LOG_INFO("FontRenderer creating render resources");
    this->init_atlas_texture(gfx);
...
FontID FontRenderer::load_font(GfxState* gfx,
        std::string path,
        u32 height,
        u32 num_glyphs,
        b8 generate_sdf) 
{
    ...
    if(!fsapi::file_exists(path.c_str())) 
    {
        LOG_WARNING("{} either does not exist or is not accesibile under current permissions level", path);
        return -1;
    }
    ...
    FT_Face face;
    FT_Error check;
    check = FT_New_Face(this->freetype_library, path.c_str(), 0, &face);

    if(check != FT_Err_Ok) 
    {
        LOG_WARNING("error loading {}: {}", path, FT_Error_String(check));
        return -1;
    }
    ...
    if(check !=  FT_Err_Ok) 
    {
        LOG_WARNING("FT_Set_Pixel_Sizes on {}: {}", path, FT_Error_String(check));
        return -1;
    }
    ...
    for(i32 i = Start_Glyph; i < num_glyphs; ++i) {
        ...
        if(check != FT_Err_Ok) {
            LOG_WARNING("FT_Load_Glyph on {}: {}", path, FT_Error_String(check));
            return -1; // can maybe continue?
        }

        if(load_params | FT_RENDER_MODE_SDF) {
            // LOG_INFO("rendering SDF glyphs for {}", path);
        } else {
            // LOG_INFO("rendering normal glyphs for {}", path);
        }

        if(Use_Bitmap_SDF) {
            check = FT_Render_Glyph(face->glyph, (FT_Render_Mode)FT_RENDER_MODE_NORMAL);
            if(check != FT_Err_Ok) {
                LOG_WARNING("FT_Render_Glyph on {}: {}", path, FT_Error_String(check));
                return -1;
            }
        }
...
void FontRenderer::draw_string(const char* str, u32 len, glm::vec2 translate, glm::vec4 color, FontID font_id) {
    FontSubregion* font;
    ...
    for(int i = 0; i < len; ++i) {
        ...
        glm::vec4 quad = this->generate_quad(str[i], font);
        glm::vec4 uvs = this->calculate_uv(str[i], font);
...
void FontRenderer::draw_string(std::string* str, glm::vec2 translate, glm::vec4 color, FontID font) {
...
glm::vec4 FontRenderer::generate_quad(const char c, FontSubregion* font) {
    glm::vec4 quad{};
...
glm::vec4 FontRenderer::calculate_uv(const char c, FontSubregion* font_subregion) {
...
void FontRenderer::generate_vertices(glm::vec4 min_max, glm::vec4 uvs, glm::vec4 color) {
    ...
    this->font_vtx_mem[this->next_free_vtx + 0] = VertexPosColorTexcoord {
        glm::vec3{min_max.x, min_max.y, 0.0f},
            color, 
            glm::vec2{uvs.x, uvs.w}
    };

    this->font_vtx_mem[this->next_free_vtx + 1] = VertexPosColorTexcoord {
        glm::vec3{min_max.z, min_max.y, 0.0f},
            color, 
            glm::vec2{uvs.z, uvs.w}
    };


    this->font_vtx_mem[this->next_free_vtx + 2] = VertexPosColorTexcoord {
        glm::vec3{min_max.x, min_max.w, 0.0f},
            color, 
            glm::vec2{uvs.x, uvs.y}
    };

    this->font_vtx_mem[this->next_free_vtx + 3] = VertexPosColorTexcoord {
        glm::vec3{min_max.z, min_max.w, 0.0f},
            color, 
            glm::vec2{uvs.z, uvs.y}
    };
...
void FontRenderer::submit(GfxState* gfx) {
